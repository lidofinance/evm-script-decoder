...
exports.EVMScriptParser = {
    parse(evmScript) {
        ...
        while (startIndex < evmScript.length) {
            ...
            const callDataLength = Number(getBytes(evmScript, startIndex, (startIndex += CALL_DATA_LENGTH_SIZE)));
            const callData = getBytes(evmScript, startIndex, (startIndex += CHARS_PER_BYTE * callDataLength));
            res.calls.push({
                address,
                callDataLength,
...
function getBytes(source, startIndex, endIndex) {
